      ...
      >
        <div className={`relative h-screen md:max-h-[700px]  overflow-hidden`}>
          {blok.articles.map((article, index) => {
            ...
            return (
                <ArticleTeaser currentSlide={currentSlide} index={index} article={article.content} key={article.uuid} />
            )
          })}
          {/* <!-- Slider controls --> */}
          <button type="button" onClick={()=>handleNext()} className="absolute top-0 left-0 z-30 flex items-center justify-center h-full px-4 cursor-pointer group focus:outline-none" data-carousel-prev>
